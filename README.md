...
1. **Reproducible Notebooks.**  The notebooks should be made in a way that anyone else can take that notebook, rerun it with the same inputs, and get the same outputs. This involves multiple details, some of which the next best practice elaborates on: The notebook should be executable from top to bottom, the notebook should contain the information required to setup the correct environment.
